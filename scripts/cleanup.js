...
async function deleteEvictedPods() {
  ...
  const pods = JSON.parse(raw).items.filter((p) => p.status?.reason === "Evicted");
...
async function deleteOldBuilds(namespace) {
  info(`Scanning builds in namespace "${namespace}" (keeping latest ${BUILDS_TO_KEEP})...`);

  const raw = await run("oc", ["get", "builds", "-n", namespace, "-o", "json"]);
  ...
  for (const [bc, builds] of Object.entries(byConfig)) {
    const sorted   = builds.sort((a, b) => new Date(a.metadata.creationTimestamp) - new Date(b.metadata.creationTimestamp));
...
async function cleanup(namespace) {
  ...
  console.log("");
  await deleteOldBuilds(namespace);
}
